...
time.sleep(2)
#irc.send("NS IDENTIFY "+raw_input("Password: "))
...
def drawTable():
	...
	for i in range(0, 9):
		sendPrivateMessage(username, "| "+gameStats["table"][i+1]+" | "+gameStats["table"][i+1]+" | "+gameStats["table"][i+1]+" |")
	sendPrivateMessage(username, "+-----------+")
...
while 1:
	time.sleep(0.1)
	gameover()
	output = irc.recv(4096)
	if output.find ( 'PING' ) != -1:
		irc.send("PONG "+output.split(" ")[1]+'\r\n')
		print("PONG "+output.split(" ")[1])
	output = output.strip('\n\r')
	print(output)
	try:
